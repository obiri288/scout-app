...
const ReportModal = ({ targetId, targetType, onClose, session }) => {
    ...
    const handleReport = async () => {
        setLoading(true);
        try { await supabase.from('reports').insert({ reporter_id: session.user.id, target_id: targetId, target_type: targetType, reason: reason, status: 'pending' }).catch(() => {}); alert("Gemeldet. Danke!"); onClose(); } catch (e) { alert("Fehler."); } finally { setLoading(false); }
    };
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in p-4">
            <div className="w-full max-w-xs bg-zinc-900 border border-zinc-800 rounded-xl p-4 shadow-2xl">
                <h3 className="font-bold text-white mb-4 flex items-center gap-2"><Flag size={16} className="text-red-500"/> Melden</h3>
                <select value={reason} onChange={e => setReason(e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 text-white p-2 rounded-lg mb-4 outline-none"><option>Spam / Werbung</option><option>Unangemessener Inhalt</option><option>Beleidigung</option><option>Fake Profil</option></select>
                <div className="flex gap-2"><button onClick={onClose} className="flex-1 bg-zinc-800 text-white py-2 rounded-lg font-bold text-xs">Abbruch</button><button onClick={handleReport} disabled={loading} className="flex-1 bg-red-600 text-white py-2 rounded-lg font-bold text-xs">Senden</button></div>
            </div>
        </div>
    );
};
      ...
      <div className="w-full max-w-sm bg-zinc-900 rounded-2xl p-8 border border-zinc-800 shadow-2xl relative">
        ...
        {successMsg ? (
            <div className="text-center space-y-4 animate-in fade-in">
                <div className="bg-green-900/30 text-green-400 p-4 rounded-xl border border-green-800 text-sm">{successMsg}</div>
...
const InboxScreen = ({ session, onSelectChat, onUserClick }) => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            if(map.size>0) { const {data:users} = await supabase.from('players_master').select('*').in('user_id', [...map.keys()]); setChats(users.map(u=>({...u, lastMsg: map.get(u.user_id).content}))); }
        }; load();
    }, [session]);
    return (
        <div className="pb-24 pt-4 px-4 max-w-md mx-auto min-h-screen">
            <h2 className="text-2xl font-bold text-white mb-4">Inbox</h2>
            <div className="space-y-2">{chats.map(c=><div key={c.id} onClick={()=>onSelectChat(c)} className="flex items-center gap-3 bg-zinc-900 p-3 rounded-xl cursor-pointer"><div onClick={(e)=>{e.stopPropagation(); onUserClick(c)}} className="w-12 h-12 rounded-full bg-zinc-800 overflow-hidden">{c.avatar_url?<img src={c.avatar_url} className="w-full h-full object-cover"/>:<User className="m-3 text-zinc-500"/>}</div><div className="flex-1"><div className="font-bold text-white">{c.full_name}</div><div className="text-xs text-zinc-400 truncate">{c.lastMsg}</div></div></div>)}</div>
        </div>
    )
};
...
const ProfileScreen = ({ player, highlights, onVideoClick, isOwnProfile, onBack, onLogout, onEditReq, onChatReq, onSettingsReq, onFollow, onShowFollowers }) => {
    ...
    return (
        <div className="pb-24 animate-in fade-in">
             <div className="bg-zinc-900 pb-6 pt-10 px-6 border-b border-zinc-800 text-center relative">
                 {!isOwnProfile && <button onClick={onBack} className="absolute left-4 top-4 text-white"><ArrowLeft size={24}/></button>}
                 {isOwnProfile && <button onClick={onSettingsReq} className="absolute right-4 top-4 text-white"><Settings size={24}/></button>}
                 <div className="w-24 h-24 rounded-full bg-zinc-800 mx-auto mb-3 overflow-hidden border-4 border-zinc-700">{player.avatar_url ? <img src={player.avatar_url} className="w-full h-full object-cover" /> : <User size={40} className="m-6 text-zinc-500"/>}</div>
                 <h1 className="text-2xl font-bold text-white flex justify-center gap-2 items-center">{player.full_name} {player.is_verified && <CheckCircle size={18} className="text-blue-500"/>}</h1>
                 <p className="text-zinc-400 text-sm mb-4">{player.clubs?.name || "Vereinslos"} • {player.position_primary}</p>
                 
                 <div className="flex justify-center gap-8 text-sm mb-6 border-y border-zinc-800 py-3">
                    <div onClick={onShowFollowers} className="cursor-pointer hover:opacity-70"><span className="font-bold text-white block text-lg">{player.followers_count||0}</span><span className="text-zinc-500 text-xs">Follower</span></div>
                    <div><span className="font-bold text-white block text-lg">{highlights.length}</span><span className="text-zinc-500 text-xs">Clips</span></div>
                 </div>

                 {isOwnProfile ? (<button onClick={onEditReq} className="w-full bg-zinc-800 text-white py-2 rounded-lg font-bold text-sm">Profil bearbeiten</button>) : (
                    <div className="flex gap-2">
                        <button onClick={onFollow} className={`flex-1 py-2 rounded-lg font-bold text-sm ${player.isFollowing ? 'bg-zinc-800 text-white' : 'bg-indigo-600 text-white'}`}>{player.isFollowing ? 'Gefolgt' : 'Folgen'}</button>
                        <button onClick={onChatReq} className="flex-1 bg-zinc-800 text-white py-2 rounded-lg font-bold text-sm">Nachricht</button>
                    </div>
                 )}
             </div>
             <div className="grid grid-cols-3 gap-0.5">{highlights.map(v => (<div key={v.id} onClick={() => onVideoClick(v)} className="aspect-[3/4] bg-zinc-800 relative cursor-pointer"><video src={v.video_url} className="w-full h-full object-cover opacity-80" /></div>))}</div>
        </div>
    )
}
...
const App = () => {
  ...
  useEffect(() => {
      if(!session?.user) return;
      const c = supabase.channel('global_user').on('postgres_changes', {event:'INSERT', schema:'public', table:'notifications'}, p => {
          if(p.new.receiver_id === session.user.id) addToast(p.new.type === 'like' ? 'Neuer Like!' : 'Neuer Follower!', 'info');
      }).on('postgres_changes', {event:'INSERT', schema:'public', table:'direct_messages'}, p => {
          if(p.new.receiver_id === session.user.id && activeChatPartnerRef.current?.user_id !== p.new.sender_id) {
              setUnreadCount(prev => prev + 1); addToast('Neue Nachricht', 'message');
          }
      }).subscribe();
      return () => { supabase.removeChannel(c); }
  }, [session]);

  const addToast = (content, type = 'info') => { const id = Date.now(); setToasts(p => [...p, { id, content, type }]); setTimeout(() => setToasts(p => p.filter(t => t.id !== id)), 4000); };
  ...
  const toggleFollow = async () => {
      ...
      if(!viewedProfile?.user_id) return addToast("Profil fehlerhaft.", "error");
      
      const old = viewedProfile.isFollowing;
      setViewedProfile(p => ({ ...p, isFollowing: !old, followers_count: (p.followers_count||0) + (!old?1:-1) }));
      ...
      try {
          ...
          loadProfile(viewedProfile); // Rollback
      }
  };

  return (
    <div className="min-h-screen bg-zinc-950 text-white font-sans selection:bg-indigo-500/30">
      {!session && <button onClick={() => setShowLogin(true)} className="fixed top-4 right-4 z-50 bg-indigo-600 text-white px-4 py-2 rounded-full text-xs font-bold shadow-lg"><LogIn size={14} /> Login</button>}
      ...
      {activeTab === 'profile' && <ProfileScreen player={viewedProfile} highlights={profileHighlights} onVideoClick={setActiveVideo} isOwnProfile={session && viewedProfile?.user_id === session.user.id} onBack={() => setActiveTab('home')} onLogout={() => supabase.auth.signOut().then(() => setActiveTab('home'))} onEditReq={() => setShowEditProfile(true)} onSettingsReq={() => setShowSettings(true)} onChatReq={() => { if(!session) setShowLogin(true); else setActiveChatPartner(viewedProfile); }} onClubClick={()=>{}} onAdminReq={()=>setActiveTab('admin')} onFollow={toggleFollow} onShowFollowers={() => setShowFollowersModal(true)} />}
      ...
      <div className="fixed bottom-0 w-full bg-zinc-950/90 border-t border-zinc-800 px-6 py-3 flex justify-between items-center z-40 pb-6 sm:pb-3">
          <button onClick={() => setActiveTab('home')} className={activeTab === 'home' ? 'text-white' : 'text-zinc-600'}><Home size={24} /></button>
          <button onClick={() => setActiveTab('search')} className={activeTab === 'search' ? 'text-white' : 'text-zinc-600'}><Search size={24} /></button>
          ...
          <button onClick={() => { setActiveTab('inbox'); setUnreadCount(0); }} className={`relative ${activeTab === 'inbox' ? 'text-white' : 'text-zinc-600'}`}><Mail size={24} />{unreadCount > 0 && <span className="absolute -top-1 -right-1 bg-red-500 w-3 h-3 rounded-full"></span>}</button>
          <button onClick={handleProfileTabClick} className={activeTab === 'profile' ? 'text-white' : 'text-zinc-600'}><User size={24} /></button>
      </div>
