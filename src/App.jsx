...
const ReportModal = ({ targetId, targetType, onClose, session }) => {
    ...
    const [loading, setLoading] = useState(false);
    const handleSubmit = async () => { setLoading(true); try { const { error } = await supabase.from('reports').insert({ reporter_id: session.user.id, target_id: targetId, target_type: targetType, reason: reason }); if (error) throw error; alert("Danke! Wir prüfen das."); onClose(); } catch (e) { alert("Fehler: " + e.message); } finally { setLoading(false); } };
    return (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in p-4">
            <div className="w-full max-w-sm bg-zinc-900 rounded-xl p-6 border border-zinc-800 shadow-2xl">
                <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><Flag size={20} className="text-red-500"/> Inhalt melden</h3>
                <div className="space-y-3"><p className="text-sm text-zinc-400">Warum möchtest du das melden?</p>{['Spam / Werbung', 'Beleidigung / Hass', 'Nacktheit / Gewalt', 'Falsche Informationen'].map(r => (<button key={r} onClick={() => setReason(r)} className={`w-full text-left p-3 rounded-lg text-sm transition ${reason === r ? 'bg-red-500/20 text-red-400 border border-red-500/50' : 'bg-zinc-800 text-zinc-300 hover:bg-zinc-700'}`}>{r}</button>))}<div className="flex gap-3 mt-4 pt-2"><button onClick={onClose} className="flex-1 bg-zinc-800 text-white py-2 rounded-lg text-sm font-bold">Abbruch</button><button onClick={handleSubmit} disabled={loading} className="flex-1 bg-red-600 text-white py-2 rounded-lg text-sm font-bold flex justify-center">{loading ? <Loader2 className="animate-spin" size={16}/> : 'Melden'}</button></div></div>
            </div>
        </div>
    );
};
...
const OnboardingWizard = ({ session, onComplete }) => {
    ...
    const [loading, setLoading] = useState(false);
    const [formData, setFormData] = useState({ full_name: '', position_primary: 'ZM', strong_foot: 'Rechts', height_user: '', club_id: null });
    ...
    useEffect(() => {
        ...
        const t = setTimeout(async () => { const { data } = await supabase.from('clubs').select('*').ilike('name', `%${clubSearch}%`).limit(5); setClubResults(data || []); }, 300);
        return () => clearTimeout(t);
    }, [clubSearch]);

    const handleCreateClub = async () => {
        if(!newClubData.name) return; setLoading(true);
        try { const { data, error } = await supabase.from('clubs').insert({ name: newClubData.name, league: newClubData.league }).select().single(); if(error) throw error; setSelectedClub(data); setShowCreateClub(false); } catch(e) { alert("Fehler: " + e.message); } finally { setLoading(false); }
    }
    ...
    const handleFinish = async () => {
        ...
        try {
            ...
            const { error } = await supabase.from('players_master').update({ full_name: formData.full_name, position_primary: formData.position_primary, strong_foot: formData.strong_foot, height_user: heightValue, club_id: selectedClub ? selectedClub.id : null, transfer_status: 'Gebunden' }).eq('user_id', session.user.id);
            if(error) throw error; onComplete();
        } catch(e) { alert("Fehler: " + e.message); } finally { setLoading(false); }
    };

    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-xl animate-in fade-in">
            <div className="w-full max-w-md bg-zinc-900 rounded-2xl p-8 border border-zinc-800 shadow-2xl relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1 bg-zinc-800"><div className="h-full bg-indigo-500 transition-all duration-500" style={{width: `${(step/3)*100}%`}}></div></div>
                <div className="mb-8 text-center"><div className="w-16 h-16 bg-indigo-500/20 text-indigo-400 rounded-full flex items-center justify-center mx-auto mb-4 animate-bounce">{step === 1 && <User size={32} />}{step === 2 && <Activity size={32} />}{step === 3 && <Shield size={32} />}</div><h2 className="text-2xl font-bold text-white mb-2">{step === 1 && "Wer bist du?"}{step === 2 && "Deine Daten"}{step === 3 && "Dein Verein"}</h2><p className="text-zinc-400 text-sm">Schritt {step} von 3</p></div>
                {step === 1 && (<div className="space-y-4 animate-in slide-in-from-right"><div><label className="text-xs text-zinc-500 font-bold uppercase ml-1">Vor- & Nachname</label><input value={formData.full_name} onChange={e=>setFormData({...formData, full_name: e.target.value})} placeholder="z.B. Kevin Kreisler" className="w-full bg-zinc-800 text-white p-4 rounded-xl outline-none border border-zinc-700 focus:border-indigo-500 text-lg" autoFocus /></div><button disabled={!formData.full_name} onClick={()=>setStep(2)} className="w-full bg-white text-black font-bold py-4 rounded-xl mt-4 flex items-center justify-center gap-2 hover:bg-zinc-200 disabled:opacity-50 transition">Weiter <ArrowRight size={20}/></button></div>)}
                {step === 2 && (<div className="space-y-4 animate-in slide-in-from-right"><div className="grid grid-cols-2 gap-4"><div><label className="text-xs text-zinc-500 font-bold uppercase">Position</label><select value={formData.position_primary} onChange={e=>setFormData({...formData, position_primary: e.target.value})} className="w-full bg-zinc-800 text-white p-3 rounded-lg mt-1 outline-none">{['TW', 'IV', 'RV', 'LV', 'ZDM', 'ZM', 'ZOM', 'RA', 'LA', 'ST'].map(p=><option key={p}>{p}</option>)}</select></div><div><label className="text-xs text-zinc-500 font-bold uppercase">Fuß</label><select value={formData.strong_foot} onChange={e=>setFormData({...formData, strong_foot: e.target.value})} className="w-full bg-zinc-800 text-white p-3 rounded-lg mt-1 outline-none"><option>Rechts</option><option>Links</option><option>Beidfüßig</option></select></div><div className="col-span-2"><label className="text-xs text-zinc-500 font-bold uppercase">Größe (cm)</label><input type="number" placeholder="185" value={formData.height_user} onChange={e=>setFormData({...formData, height_user: e.target.value})} className="w-full bg-zinc-800 text-white p-3 rounded-lg mt-1 outline-none" /></div></div><div className="flex gap-3"><button onClick={()=>setStep(1)} className="px-6 py-4 rounded-xl bg-zinc-800 text-white font-bold">Zurück</button><button onClick={()=>setStep(3)} className="flex-1 bg-white text-black font-bold py-4 rounded-xl flex items-center justify-center gap-2 hover:bg-zinc-200 transition">Weiter <ArrowRight size={20}/></button></div></div>)}
                {step === 3 && (<div className="space-y-4 animate-in slide-in-from-right">{selectedClub ? (<div className="flex items-center justify-between bg-zinc-800 p-4 rounded-xl border border-indigo-500/50"><div className="flex items-center gap-3">{selectedClub.is_verified ? <img src={selectedClub.logo_url} className="w-10 h-10 object-contain"/> : <ShieldAlert size={32} className="text-zinc-500" /> }<div><div className="font-bold text-white">{selectedClub.name}</div><div className="text-xs text-zinc-400">{selectedClub.is_verified ? selectedClub.league : "Pending"}</div></div></div><button onClick={() => { setSelectedClub(null); setClubSearch(''); setShowCreateClub(false); }}><X size={20} className="text-zinc-400 hover:text-white"/></button></div>) : showCreateClub ? (<div className="bg-zinc-800 p-4 rounded-xl border border-zinc-700 space-y-3"><h4 className="text-sm font-bold text-white">Neuen Verein anlegen</h4><input placeholder="Vereinsname" value={newClubData.name} onChange={e=>setNewClubData({...newClubData, name: e.target.value})} className="w-full bg-zinc-900 text-white p-3 rounded border border-zinc-700 text-sm" /><select value={newClubData.league} onChange={e=>setNewClubData({...newClubData, league: e.target.value})} className="w-full bg-zinc-900 text-white p-3 rounded border border-zinc-700 text-sm"><option>Kreisliga</option><option>Bezirksliga</option><option>Landesliga</option><option>Verbandsliga</option><option>Oberliga</option><option>Regionalliga</option><option>Jugendliga</option></select><button onClick={handleCreateClub} className="w-full bg-indigo-600 text-white font-bold py-3 rounded-lg">Erstellen & Wählen</button></div>) : (<div className="relative"><input placeholder="Suche deinen Verein..." value={clubSearch} onChange={e => setClubSearch(e.target.value)} className="w-full bg-zinc-800 text-white p-4 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" autoFocus />{clubSearch.length > 1 && (<div className="absolute z-10 w-full bg-zinc-900 border border-zinc-800 rounded-xl mt-2 shadow-xl overflow-hidden max-h-48 overflow-y-auto">{clubResults.map(c => (<div key={c.id} onClick={() => { setSelectedClub(c); setClubSearch(''); }} className="p-3 hover:bg-zinc-800 cursor-pointer flex items-center gap-3">{c.is_verified ? <img src={c.logo_url} className="w-6 h-6 object-contain"/> : <Shield size={20} className="text-zinc-500"/>}<div className="text-sm text-white">{c.name}</div></div>))}<div onClick={() => { setNewClubData({...newClubData, name: clubSearch}); setShowCreateClub(true); }} className="p-3 hover:bg-zinc-800 cursor-pointer border-t border-zinc-800 flex items-center gap-2 text-indigo-400"><Plus size={16} /><span className="text-sm font-bold">"{clubSearch}" erstellen</span></div></div>)}</div>)}<div className="flex gap-3 mt-6"><button onClick={()=>setStep(2)} className="px-6 py-4 rounded-xl bg-zinc-800 text-white font-bold">Zurück</button><button onClick={handleFinish} disabled={loading} className="flex-1 bg-indigo-600 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 hover:bg-indigo-700 transition">{loading ? <Loader2 className="animate-spin"/> : "Abschließen & Starten 🚀"}</button></div>{!selectedClub && <p className="text-center text-xs text-zinc-500 mt-2">Du kannst den Verein auch später im Profil hinzufügen.</p>}</div>)}
            </div>
        </div>
    );
};
...
const EditProfileModal = ({ player, onClose, onUpdate }) => {
  ...
  const handleSave = async (e) => { e.preventDefault(); setLoading(true); try { let finalAvatarUrl = player.avatar_url; if (avatarFile) { const filePath = `${player.user_id}/${Date.now()}.jpg`; const { error: upErr } = await supabase.storage.from('avatars').upload(filePath, avatarFile); if (upErr) throw upErr; const { data: { publicUrl } } = supabase.storage.from('avatars').getPublicUrl(filePath); finalAvatarUrl = publicUrl; } const heightValue = formData.height_user ? parseInt(formData.height_user) : null; const { data, error } = await supabase.from('players_master').update({ ...formData, height_user: heightValue, avatar_url: finalAvatarUrl, club_id: selectedClub ? selectedClub.id : null }).eq('id', player.id).select('*, clubs(*)').single(); if (error) throw error; onUpdate(data); onClose(); } catch (err) { alert('Fehler: ' + err.message); } finally { setLoading(false); } };
  ...
  return (
    <div className="bg-zinc-900 border-b border-zinc-800 pb-6 mb-6 last:border-0 relative">
      <div className="flex items-center justify-between px-4 py-3">
        <div className="flex items-center gap-3">
          <div className="relative cursor-pointer" onClick={()=>onUserClick(player)}>
             <div className="w-10 h-10 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center overflow-hidden">{player?.avatar_url ? <img src={player.avatar_url} className="w-full h-full object-cover" /> : <span className="font-bold text-zinc-500">{player?.full_name?.charAt(0)}</span>}</div>
      ...
      <div className="px-4 py-3 flex items-center gap-6">
          <button onClick={handleLike} className={`flex items-center gap-2 transition ${isLiked?'text-red-500':'text-white'}`}><Heart size={26} className={isLiked?"fill-red-500":""}/><span className="font-bold text-sm">{likes}</span></button>
          ...
          <div className="ml-auto text-zinc-400 flex items-center gap-1"><Activity size={18} /><span className="text-xs">{video.views_count}</span></div>
      </div>
    </div>
  );
};
...
const ClubScreen = ({ club, onBack, onUserClick }) => {
    ...
    const handleInvite = async () => { const text = `Hey! Ich habe ${club.name} auf der neuen Fußball-App erstellt. Komm ins Team, wir brauchen 11 Mann für die Verifizierung! 🚀`; const url = window.location.href; if (navigator.share) { try { await navigator.share({ title: club.name, text: text, url: url }); } catch(e) {} } else { alert("Link kopiert! Schick ihn in die WhatsApp Gruppe."); } };
    ...
    return (
        <div className="pb-24 animate-in fade-in min-h-screen">
             <div className="bg-zinc-900 pb-8 pt-10 px-6 border-b border-zinc-800 text-center relative"><button onClick={onBack} className="absolute left-4 top-4 p-2 bg-white/10 rounded-full text-white"><ArrowLeft size={20}/></button><div className="w-24 h-24 mx-auto mb-4 bg-white rounded-full p-4 shadow-lg flex items-center justify-center">{club.is_verified ? <img src={club.logo_url} className="w-full h-full object-contain"/> : <Shield size={64} className="text-zinc-400" />}</div><h1 className="text-2xl font-bold text-white flex items-center justify-center gap-2">{club.name} {club.is_verified && <CheckCircle size={20} className="text-blue-500 fill-blue-500/20"/>}</h1><p className="text-zinc-400 text-sm mb-6">{club.league || "Amateur"}</p>{!club.is_verified && (<div className="bg-zinc-800/50 p-4 rounded-xl border border-zinc-700/50 mb-6"><div className="flex justify-between items-end mb-2"><span className="text-xs font-bold text-zinc-400 uppercase">Verifizierungs-Status</span><span className="text-sm font-bold text-white">{memberCount} / {needed} Spieler</span></div><div className="w-full h-2 bg-zinc-700 rounded-full overflow-hidden"><div className="h-full bg-indigo-500 transition-all duration-1000 ease-out" style={{width: `${progress}%`}}></div></div><p className="text-[10px] text-zinc-500 mt-2 text-left">Lade 11 Spieler ein, um das echte Wappen freizuschalten.</p><button onClick={handleInvite} className="w-full mt-3 bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-lg flex items-center justify-center gap-2 transition animate-pulse"><Share2 size={18} /> Mannschaft einladen</button></div>)}{club.is_verified && (<div className="flex justify-center gap-4 text-sm font-bold text-zinc-300"><div className="bg-zinc-800 px-4 py-2 rounded-lg">{squad.length} Spieler</div></div>)}</div>
             <div className="p-4"><h3 className="text-zinc-500 text-xs font-bold uppercase mb-3 ml-1">Kader Übersicht</h3><div className="space-y-2">{squad.map(p => (<div key={p.id} onClick={()=>onUserClick(p)} className="flex items-center gap-4 bg-zinc-900/50 p-3 rounded-xl border border-zinc-800/50 cursor-pointer hover:bg-zinc-800 transition"><div className="w-10 h-10 rounded-full bg-zinc-800 flex items-center justify-center overflow-hidden">{p.avatar_url?<img src={p.avatar_url} className="w-full h-full object-cover"/>:<span className="font-bold text-zinc-500">{p.full_name?.charAt(0)}</span>}</div><div className="flex-1"><h4 className="text-white font-bold text-sm">{p.full_name}</h4><span className="text-xs text-zinc-500 bg-zinc-950 px-1.5 py-0.5 rounded">{p.position_primary}</span></div><ChevronRight size={16} className="text-zinc-600"/></div>))}{squad.length === 0 && <div className="text-center text-zinc-500 py-10">Keine Spieler registriert.</div>}</div></div>
        </div>
    );
};

// HOME SCREEN (UPDATED: MIT TABS & FOLLOW LOGIC)
const HomeScreen = ({ onVideoClick, session, onLikeReq, onCommentClick, onUserClick, onClubClick, onReportReq }) => {
      ...
      const fetchFeed = async () => {
          ...
          if(tab === 'following' && session) {
              ...
              if(followingIds.length > 0) {
                  // 2. Get player_ids for those users
                  const { data: players } = await supabase.from('players_master').select('id').in('user_id', followingIds);
                  const playerIds = players?.map(p => p.id) || [];
                  // 3. Filter videos
                  if(playerIds.length > 0) query = query.in('player_id', playerIds);
                  else { setFeed([]); return; } // Follows no one with videos
              } else {
                  setFeed([]); return; // Follows no one
              }
          }
  ...
  return (
      <div className="pb-24 pt-4 md:pt-0 max-w-md mx-auto">
          {/* FEED TABS */}
          <div className="fixed top-0 left-0 w-full z-30 flex justify-center pt-4 pb-4 bg-gradient-to-b from-black/80 to-transparent pointer-events-none">
              <div className="flex gap-4 pointer-events-auto">
                  <button onClick={()=>setTab('for_you')} className={`text-sm font-bold drop-shadow-md ${tab==='for_you'?'text-white scale-110':'text-zinc-400'}`}>Für dich</button>
                  <div className="w-px h-4 bg-zinc-500/50 self-center"></div>
                  <button onClick={()=>{ if(!session) onLikeReq(); else setTab('following'); }} className={`text-sm font-bold drop-shadow-md ${tab==='following'?'text-white scale-110':'text-zinc-400'}`}>Gefolgt</button>
              </div>
          </div>
          
          <div className="pt-0">
              {feed.length > 0 ? feed.map(v => <FeedItem key={v.id} video={v} onClick={onVideoClick} session={session} onLikeReq={onLikeReq} onCommentClick={onCommentClick} onUserClick={onUserClick} onClubClick={onClubClick} onReportReq={onReportReq} />) : (
                  <div className="min-h-screen flex flex-col items-center justify-center text-zinc-500 pb-20">
                      <p>Keine Videos gefunden.</p>
                      {tab === 'following' && <button onClick={()=>setTab('for_you')} className="mt-4 text-indigo-400 text-sm">Entdecke neue Spieler</button>}
                  </div>
              )}
          </div>
      </div>
...
const ProfileScreen = ({ player, highlights, onVideoClick, isOwnProfile, onBack, onLogout, onEditReq, onChatReq, onClubClick, onAdminReq }) => {
    ...
    const [localFollowers, setLocalFollowers] = useState(player.followers_count || 0);

    useEffect(() => {
        const checkFollow = async () => {
            const { data: { session } } = await supabase.auth.getSession();
            if(!session) return;
            const { data } = await supabase.from('follows').select('*').eq('follower_id', session.user.id).eq('following_id', player.user_id).single();
            setIsFollowing(!!data);
        };
        if(!isOwnProfile) checkFollow();
    }, [player]);
    ...
    const handleFollow = async () => {
        ...
        setLocalFollowers(prev => newStatus ? prev + 1 : prev - 1); // Optimistic UI

        if (newStatus) {
            await supabase.from('follows').insert({ follower_id: session.user.id, following_id: player.user_id });
        } else {
            await supabase.from('follows').delete().eq('follower_id', session.user.id).eq('following_id', player.user_id);
        }
    };
    ...
    return (
        <div className="pb-24 animate-in fade-in">
             <div className="bg-gradient-to-b from-zinc-900 to-zinc-950 pb-6 pt-10 px-6 border-b border-zinc-800 text-center relative">
                 ...
                 {isOwnProfile && <button onClick={onLogout} className="absolute right-4 top-4 p-2 bg-white/10 rounded-full text-white"><LogOut size={20}/></button>}
                 <div className="w-24 h-24 rounded-full bg-zinc-800 border-4 border-zinc-900 shadow-xl mx-auto mb-3 flex items-center justify-center overflow-hidden relative">{player.avatar_url ? <img src={player.avatar_url} className="w-full h-full object-cover" /> : <span className="text-3xl font-bold text-zinc-600">{player.full_name?.charAt(0)}</span>}</div>
                 <h1 className="text-2xl font-bold text-white mb-1 flex items-center justify-center gap-2">{player.full_name} {player.is_verified && <CheckCircle size={18} className="text-blue-500 fill-blue-500/20"/>}</h1>
                 ...
                 <div className="flex justify-center gap-6 text-sm mb-6 border-t border-b border-zinc-800 py-4"><div className="flex flex-col"><span className="font-bold text-white text-lg">{localFollowers}</span><span className="text-zinc-500 text-xs uppercase">Follower</span></div><div className="flex flex-col"><span className="font-bold text-white text-lg">{highlights.length}</span><span className="text-zinc-500 text-xs uppercase">Clips</span></div></div>
                 {isOwnProfile ? (<div className="space-y-2"><button onClick={onEditReq} className="w-full bg-zinc-800 hover:bg-zinc-700 text-white py-2.5 rounded-lg font-semibold text-sm border border-zinc-700 transition">Profil bearbeiten</button>{player.is_admin && <button onClick={onAdminReq} className="w-full bg-indigo-900/20 hover:bg-indigo-900/40 text-indigo-400 border border-indigo-500/30 py-2.5 rounded-lg font-semibold text-sm transition flex items-center justify-center gap-2"><Database size={16}/> Admin Dashboard</button>}</div>) : (
                    <div className="space-y-2">
                        <div className="flex gap-2">
                            {/* FOLLOW BUTTON */}
                            <button onClick={handleFollow} className={`flex-1 py-2.5 rounded-lg font-semibold text-sm transition ${isFollowing ? 'bg-zinc-800 text-white border border-zinc-700' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}>{isFollowing ? 'Gefolgt' : 'Folgen'}</button>
                        ...
                        <button className="w-full text-red-500 text-xs font-bold py-2 flex items-center justify-center gap-1 hover:bg-red-500/10 rounded"><Ban size={14}/> Nutzer blockieren</button>
                    </div>
                 )}
             </div>
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white font-sans selection:bg-indigo-500/30">
      ...
      {activeTab === 'profile' && <ProfileScreen player={viewedProfile} highlights={profileHighlights} onVideoClick={setActiveVideo} isOwnProfile={session && viewedProfile?.user_id === session.user.id} onBack={() => setActiveTab('home')} onLogout={() => supabase.auth.signOut().then(() => setActiveTab('home'))} onEditReq={() => setShowEditProfile(true)} onChatReq={() => { if(!session) setShowLogin(true); else setActiveChatPartner(viewedProfile); }} onClubClick={loadClub} onAdminReq={()=>setActiveTab('admin')} />}
