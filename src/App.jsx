...
const legalContent = {
    'Datenschutz': {
        updated: '01.03.2026',
        sections: [
            { title: '1. Verantwortlicher', text: 'Verantwortlich für die Datenverarbeitung ist CAVIOS. Bei Fragen zum Datenschutz wende dich an: kontakt@cavios.de' },
    ...
    'Impressum': {
        updated: '01.03.2026',
        sections: [
            { title: 'Angaben gemäß § 5 TMG', text: 'CAVIOS Digital Sports\nMusterstraße 1\n10115 Berlin\nDeutschland' },
            { title: 'Kontakt', text: 'E-Mail: kontakt@cavios.de\nTelefon: +49 (0) 30 12345678' },
    ...
    'AGB': {
        updated: '01.03.2026',
        sections: [
            { title: '1. Geltungsbereich', text: 'Diese Nutzungsbedingungen gelten für die Nutzung der CAVIOS-App. Mit der Registrierung akzeptierst du diese Bedingungen.' },
            ...
            { title: '6. Haftungsausschluss', text: 'CAVIOS übernimmt keine Garantie für die Richtigkeit von Nutzerangaben. Die Plattform dient der Sichtbarkeit von Spielern und stellt keine Vermittlungsgarantie dar.' },
        ]
    }
};
...
const App = () => {
    ...
    return (
        <div className="min-h-screen bg-background text-foreground font-sans selection:bg-cyan-500/30 pb-32">
            ...
            {/* Notification Bell — fixed top-right (Hidden during video or admin dashboard) */}
            {session && currentUserProfile && !activeVideo && !activeTab.startsWith('admin') && (
            ...
            <div className="fixed bottom-[calc(1.5rem+env(safe-area-inset-bottom))] left-1/2 -translate-x-1/2 w-[90%] max-w-sm bg-card/80 backdrop-blur-2xl border border-border py-3 px-6 rounded-[2rem] shadow-[0_15px_40px_rgba(0,0,0,0.8),0_0_60px_rgba(16,185,129,0.05)] flex justify-between items-center z-[9999] pointer-events-auto">
                ...
                <button onClick={handleProfileTabClick} className={`relative flex items-center gap-2 p-2 rounded-full transition-all duration-500 ease-out ${activeTab === 'profile' ? 'bg-cyan-500/15 text-cyan-400 px-4' : 'text-muted-foreground hover:text-foreground/70 hover:bg-white/5'}`}>
                    ...
                    <span className={`text-sm font-medium overflow-hidden whitespace-nowrap transition-all duration-500 ${activeTab === 'profile' ? 'max-w-[120px] opacity-100 ml-1' : 'max-w-0 opacity-0'}`}>
                        {currentUserProfile?.email === 'kontakt@cavios.de' || currentUserProfile?.role === 'system' || currentUserProfile?.is_official ? 'CAVIOS Support' : 'Profil'}
                    </span>
                </button>
            </div>
