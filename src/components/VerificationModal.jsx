        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
            <div className={`w-full max-w-md ${cardStyle} p-6 border-t border-border shadow-2xl relative`}>
                ...
                <div className="space-y-6">
                    ...
                    <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-border rounded-2xl cursor-pointer hover:bg-slate-100 dark:hover:bg-zinc-800/50 hover:border-blue-500/50 transition-all group">
                        <div className="p-4 bg-slate-200 dark:bg-zinc-800 rounded-full mb-3 group-hover:scale-110 transition-transform"><FileBadge className="w-8 h-8 text-blue-500 dark:text-blue-400" /></div>
                        <p className="text-sm text-foreground dark:text-zinc-300 font-medium">Dokument auswählen</p>
                        <p className="text-xs text-muted-foreground mt-1">JPG, PNG oder PDF</p>
                        <input type="file" className="hidden" onChange={handleUpload} />
                    </label>
                    {uploading && <div className="text-center text-muted-foreground text-xs flex items-center justify-center gap-2"><Loader2 className="animate-spin" size={14} /> Upload läuft...</div>}
                </div>
            </div>
        </div>
    );
