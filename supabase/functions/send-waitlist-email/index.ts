...
serve(async (req) => {
  ...
  try {
    ...
    // CAVIO Premium HTML Mail Template
    const htmlContent = `
      <div style="background-color: #020617; color: #e2e8f0; font-family: Arial, sans-serif; padding: 40px 20px; line-height: 1.6; text-align: center;">
        <div style="max-width: 600px; margin: 0 auto; background-color: #0f172a; padding: 40px; border-radius: 12px; border: 1px solid #1e293b;">
          <img src="https://wwdfagjgnliwraqrwusc.supabase.co/storage/v1/object/public/assets/image.png" alt="CAVIO" style="max-height: 55px; width: auto; object-fit: contain; margin-bottom: 30px; display: inline-block; filter: drop-shadow(0 2px 8px rgba(34, 211, 238, 0.15));" />
          <h2 style="color: #f8fafc; font-size: 20px; margin-bottom: 24px;">Deine Position auf der CAVIO Warteliste ist bestätigt.</h2>
          <p style="text-align: left; margin-bottom: 16px;">Deine E-Mail-Adresse wurde erfolgreich in unserem System hinterlegt.</p>
          <p style="text-align: left; margin-bottom: 16px;">CAVIO ist kein gewöhnliches Netzwerk. Wir bauen die exklusivste digitale Infrastruktur für modernes Scouting – eine Plattform, auf der Leistungsdaten, verifizierte Karrierestationen und professionelle Verbindungen die Währung sind.</p>
          ...
          <p style="text-align: left; font-size: 14px; color: #94a3b8;">Sportliche Grüße,<br><strong style="color: #f8fafc;">Das CAVIO Team</strong></p>
        </div>
      </div>
    `;
    ...
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "CAVIO <kontakt@cavio.me>",
        to: [email],
        subject: "Status: Bestätigt. Deine Position auf der CAVIO Warteliste.",
        html: htmlContent,
      }),
    });
